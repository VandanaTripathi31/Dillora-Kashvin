...
import { api, BANNER_PRESETS } from '@/services/api';
...
export default function AdminCoupons() {
  ...
  const add = async () => {
    ...
    setSaving(false);
    if (res?.error) { setErr(res.error); notify(res.error, 'error'); return; }
    setForm(blankCoupon); load(); notify('Coupon created');
  };

  const toggle = async (c) => { await api.updateCoupon(c.code, { active: !c.active }); load(); notify(`${c.code} ${c.active ? 'deactivated' : 'activated'}`, 'info'); };
  const del = async (c) => {
    const ok = await confirmDialog({ title: 'Delete coupon?', message: `Code “${c.code}” will be removed and can no longer be used.`, confirmLabel: 'Delete' });
    if (ok) { await api.deleteCoupon(c.code); load(); notify('Coupon deleted', 'info'); }
  };
