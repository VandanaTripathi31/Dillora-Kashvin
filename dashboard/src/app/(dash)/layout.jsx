...
export default function DashLayout({ children }) {
  ...
  const navLink =
    "flex items-center gap-2.5 rounded-xl px-3 py-[10px] text-[0.86rem] font-semibold leading-tight transition-[background,color,transform] duration-200";
  const navOff =
    "text-white/70 hover:translate-x-0.5 hover:bg-white/[.08] hover:text-white";
  const navOn =
    "bg-[linear-gradient(135deg,rgba(166,79,214,.95),rgba(122,79,240,.95))] text-white shadow-[0_8px_20px_-6px_rgba(122,79,240,.6)]";
  ...
  return (
    <div className="grid min-h-screen grid-cols-1 bg-[#f5f2fb] min-[901px]:grid-cols-[256px_1fr]">
      ...
      <div className="sticky top-0 z-[60] flex items-center justify-between gap-3 border-b border-[#eee3f3] bg-white px-3.5 py-[11px] min-[901px]:hidden">
        <button
          className="grid h-10 w-10 cursor-pointer place-items-center rounded-[10px] border-none bg-transparent text-ink hover:bg-[#f9f2fd]"
      ...
      {open && (
        <div
          className="fixed inset-0 z-[110] animate-[admFade_0.2s_ease] bg-[rgba(20,12,30,.5)] min-[901px]:hidden"
          onClick={() => setOpen(false)}
        />
      )}

      <aside
        className={`scrollbar-hide sticky top-0 flex h-screen flex-col gap-1.5 overflow-y-auto overscroll-contain border-r border-white/[.06] bg-[linear-gradient(185deg,#3a2456_0%,#2c2336_58%,#221830_100%)] px-3.5 py-5 text-white max-[900px]:fixed max-[900px]:inset-y-0 max-[900px]:left-0 max-[900px]:z-[120] max-[900px]:w-[min(82vw,280px)] max-[900px]:border-r-0 max-[900px]:transition-transform max-[900px]:duration-[280ms] ${open ? "max-[900px]:translate-x-0 max-[900px]:shadow-[0_30px_80px_rgba(0,0,0,.45)]" : "max-[900px]:-translate-x-full"}`}
      >
        <div className="flex items-center gap-[11px] px-2 pb-5 pt-1.5">
          <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[linear-gradient(135deg,#a64fd6,#7a4ff0)] font-display text-2xl font-bold leading-none text-white shadow-[0_6px_18px_rgba(122,79,240,.45)]">
            D
          </span>
          <span className="flex flex-col leading-[1.15]">
            <strong className="font-display text-[1.2rem] font-bold text-white">
              Dillora
            </strong>
            <span className="font-body text-[0.68rem] uppercase tracking-[0.12em] text-white/50">
              Admin panel
            </span>
          </span>
          <button
            className="ml-auto hidden cursor-pointer border-none bg-transparent text-white/70 max-[900px]:block"
          ...
          {NAV.map((n) => {
            ...
            return (
              <Link
                key={n.href}
                href={n.href}
                className={`${navLink} ${active ? navOn : navOff}`}
              >
                <Icon
        ...
        <div className="mt-auto flex flex-col gap-2 border-t border-white/[.08] pt-3.5">
          {admin && (
            <div className="muted px-1 pb-2 text-[12px]">
              Signed in as <strong>{admin.email}</strong>
            </div>
          )}
          <a
            className="btn btn-block flex justify-center gap-2 !border !border-white/[.14] !bg-white/[.08] !text-white"
            href={STORE_URL}
            target="_blank"
            rel="noreferrer"
          >
          ...
          <button
            className="flex cursor-pointer items-center justify-center gap-2 border-none bg-transparent p-1.5 text-[0.8rem] text-white/45 hover:text-white/80"
            onClick={doLogout}
          >
      ...
      <main className="scrollbar-hide overflow-x-auto bg-[#f5f2fb]">
        {children}
      </main>
    </div>
  );
