...
export default function Dashboard() {
  ...
  return (
    <div className="adm__pad">
      ...
      <div className="stats">
        <Stat label="Revenue" value={`₹${revenue.toLocaleString('en-IN')}`} hint="All orders" />
        <Stat label="Orders" value={orders.length} hint={`${pending} processing`} />
        <Stat label="Products" value={products.length} hint={`${lowStock} low on stock`} />
        <Stat label="Avg. order" value={`₹${Math.round(revenue/orders.length || 0).toLocaleString('en-IN')}`} hint="Per order" />
      </div>
      ...
      <div className="adm__row">
        ...
        <section className="card adm__panel">
          <div className="adm__panelhead">
            <h3>Recent orders</h3>
            <Link href="/admin/orders" className="adm__seeall">See all →</Link>
          </div>
          <table className="tbl">
